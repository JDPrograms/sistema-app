...
export default function SiteAdminSecurityPage() {
  ...
  useEffect(() => {
    fetch(`/api/site/${slug}/admins/mfa`)
  ...
  async function startSetup() {
    setError("");
    const res = await fetch(`/api/site/${slug}/admins/mfa/setup`);
  ...
  async function verifyAndEnable(e: React.FormEvent) {
    ...
    setError("");
    setSaving(true);
    const res = await fetch(`/api/site/${slug}/admins/mfa/enable`, {
      method: "POST",
    ...
    if (res.ok) {
      setStatus("enabled");
      setCode("");
      setSuccess("MFA activado. Se pedirá en el próximo inicio de sesión.");
    } else {
  ...
  async function disableMfa(e: React.FormEvent) {
    ...
    setError("");
    setSaving(true);
    const res = await fetch(`/api/site/${slug}/admins/mfa/disable`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="p-6 md:p-8 max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">Protege tu cuenta con una app como Google Authenticator o Authy</p>
            </div>
          </div>
        ...
        {status === "loading" && <p className="text-sm text-gray-400">Cargando...</p>}

        {status === "disabled" && (
          <div>
            <p className="text-sm text-gray-600 mb-4">
              Al activar MFA, necesitarás escanear un código QR con una app de autenticación.
            </p>
        ...
        {status === "setup" && (
          <div className="space-y-4">
            <p className="text-sm text-gray-600">1. Escanea este código QR con tu aplicación de autenticación</p>
            {qrDataUrl && (
            ...
            <p className="text-sm text-gray-600">2. Ingresa el código de 6 dígitos para confirmar:</p>
        ...
        {status === "enabled" && (
          <div>
            <p className="text-sm text-gray-600 mb-4">
              MFA está activo. Se te pedirá un código cada vez que inicies sesión.
            </p>
        ...
        {status === "disabling" && (
          <div className="space-y-4">
            <p className="text-sm text-gray-600">Para confirmar, ingresa el código de tu app de autenticación:</p>
