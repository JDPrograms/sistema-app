...
export default function OnboardingPage() {
  ...
  function addService() {
    setServices((p) => [...p, { name: "", price: "", duration: "" }]);
  }
  ...
  function addStaff() {
    setStaff((p) => [...p, { name: "", specialty: "" }]);
  }
  function removeStaff(i: number) {
    setStaff((p) => p.filter((_, idx) => idx !== i));
  }
              ...
              {services.map((svc, i) => (
                <div key={i} className="bg-slate-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-xl p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    {services.length > 1 && (
                      <button onClick={() => removeService(i)} className="text-red-400 hover:text-red-500 text-xs transition-colors">
                        Eliminar
                      </button>
                    )}
                  </div>
              ...
              {!skipStaff && staff.map((member, i) => (
                <div key={i} className="bg-slate-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-xl p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    {staff.length > 1 && (
                      <button onClick={() => removeStaff(i)} className="text-red-400 hover:text-red-500 text-xs transition-colors">
                        Eliminar
                      </button>
                    )}
                  </div>
