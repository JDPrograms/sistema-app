...
interface AuditLog { id: string; action: string; actorName?: string; actorRole?: string; entityType?: string; entityId?: string; details?: string; ip?: string; createdAt: string }
...
export default function AuditLogPage() {
  ...
  async function load(p: number) {
    setLoading(true);
    const res = await fetch(`/api/site/${slug}/audit-log?page=${p}&limit=${limit}`);
  ...
  return (
    <div className="p-6 max-w-5xl">
      <h1 className="text-2xl font-bold text-gray-900 mb-6">Registro de actividad</h1>
