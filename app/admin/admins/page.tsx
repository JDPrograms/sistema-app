...
import { SUPERADMIN_PERMS } from "@/lib/permissions";

type PermKey = keyof typeof SUPERADMIN_PERMS;
...
const EMPTY_PERMS = Object.fromEntries(
  Object.keys(SUPERADMIN_PERMS).map((k) => [k, false])
...
export default function SuperAdminsPage() {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    if (editingId) {
      ...
      const r = await fetch(`/api/admin/superadmins/${editingId}`, {
  ...
  async function handleDelete(id: string, name: string) {
    if (!confirm(`Eliminar superadmin "${name}"? Esta acción no se puede deshacer.`)) return;
    const r = await fetch(`/api/admin/superadmins/${id}`, { method: "DELETE" });
  ...
  return (
    <div className="p-4 sm:p-8">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Superadministradores</h1>
      ...
      {showPanel && (
        <form onSubmit={handleSave} className="bg-white rounded-xl border border-gray-200 p-6 mb-6 space-y-5">
          <h2 className="font-semibold text-gray-900">{isEditing ? "Editar superadmin" : "Nuevo superadmin"}</h2>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {(Object.entries(SUPERADMIN_PERMS) as [PermKey, string][]).map(([key, label]) => (
      ...
      <div className="bg-white rounded-xl border border-gray-200">
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-400">No hay superadmins registrados.</div>
        ) : (
          <div className="divide-y divide-gray-50">
            {admins.map((a) => {
              const p = parsePerms(a.permissions);
              const activePerms = a.isMaster
                ? Object.values(SUPERADMIN_PERMS)
                : (Object.entries(SUPERADMIN_PERMS) as [PermKey, string][])
                  ...
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-10 h-10 rounded-full flex items-center justify-center font-bold text-white flex-shrink-0"
                        style={{ backgroundColor: a.isMaster ? "#2563eb" : "#6b7280" }}>
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {a.isMaster && (
                            <span className="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full font-medium">Master</span>
                          )}
                        </div>
