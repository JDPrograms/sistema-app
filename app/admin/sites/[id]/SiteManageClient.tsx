...
interface Site {
  ...
  pwaEnabled: boolean; pwaShortName?: string | null;
  twaPackageName?: string | null; twaFingerprint?: string | null;
...
export default function SiteManageClient({ site }: { site: Site }) {
  ...
  const [pwaEnabled, setPwaEnabled] = useState(site.pwaEnabled ?? false);
  const [pwaShortName, setPwaShortName] = useState(site.pwaShortName ?? "");
  const [twaPackageName, setTwaPackageName] = useState(site.twaPackageName ?? "");
  const [twaFingerprint, setTwaFingerprint] = useState(site.twaFingerprint ?? "");
  ...
  async function handleSavePwa() {
    ...
    const res = await fetch(`/api/admin/sites/${site.id}`, {
      ...
      body: JSON.stringify({
        ...
        twaPackageName: twaPackageName.trim() || null,
        twaFingerprint: twaFingerprint.trim() || null,
      }),
    });
    setSavingPwa(false);
    if (res.ok) { setPwaMsg("Guardado correctamente"); setTimeout(() => setPwaMsg(""), 3000); }
    else setPwaMsg("Error al guardar");
  }
